...
public class ItemRendererSortingPreciousChest implements IItemRenderer
{
    private static ResourceLocation[] textures;

    public ItemRendererSortingPreciousChest()
    {
    ...
    public void renderItem(ItemRenderType type, ItemStack item, Object... data)
    {
        if (textures == null)
        {
            textures = ReflectionHelper.getPrivateValue(TileEntityPreciousChestRenderer.class, new TileEntityPreciousChestRenderer(), "textures");
        }
        render(item, 0);
        render(item, 1);
    }
    ...
    public void render(ItemStack itemStack, int renderPass)
    {
        ...
        if (renderPass == 0)
        {
            ResourceLocation loc = textures[itemStack.getItemDamage()];
            if (loc != null)
            {
